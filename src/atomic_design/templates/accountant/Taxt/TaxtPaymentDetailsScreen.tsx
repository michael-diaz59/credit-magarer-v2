...
export const TaxtPaymentDetailsScreen: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const theme = useTheme();
  const navigate = useNavigate();
  const companyId = useAppSelector((state) => state.user.user?.companyId ?? "");

  const [payment, setPayment] = useState<TaxtPayment | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchPayment = async () => {
      ...
      const result = await orchestrator.getTaxtPaymentById({ companyId, id });
      ...
      if (result.ok && result.value.payment) {
        ...
        if (result.value.payment.idProofOfPayment) {
          try {
            const path = `companies/${companyId}/taxtPayments/${id}/${result.value.payment.idProofOfPayment}`;
